...
--!
--! Remove any dependencies so we can move tables between schemas without risk of error
--!
if objectpropertyex(object_id(N'log4.FK_Journal_Severity'), N'IsForeignKey') is not null
	alter table log4.Journal drop constraint FK_Journal_Severity ;
go
if objectpropertyex(object_id(N'log4.FK_Journal_Exception'), N'IsForeignKey') is not null
	alter table log4.Journal drop constraint FK_Journal_Exception ;
go
if objectpropertyex(object_id(N'log4.FK_Journal_SqlException'), N'IsForeignKey') is not null
	alter table log4.Journal drop constraint FK_Journal_SqlException ;
go
if objectpropertyex(object_id(N'log4.FK_JournalDetail_Journal'), N'IsForeignKey') is not null
	alter table log4.JournalDetail drop constraint FK_JournalDetail_Journal ;
go

if objectpropertyex(object_id(N'log4.PK_Journal'), N'IsPrimaryKey') is not null
	alter table log4.Journal drop constraint PK_Journal;
go
if indexproperty(object_id(N'log4.Journal'), N'NCI_Journal_FunctionName_Severity', N'IndexID') is not null
	drop index log4.Journal.NCI_Journal_FunctionName_Severity;
go
if indexproperty(object_id(N'log4.Journal'), N'CI_Journal_SystemDate', N'IndexID') is not null
	drop index log4.Journal.CI_Journal_SystemDate;
go

if objectpropertyex(object_id(N'log4.PK_JournalControl'), N'IsPrimaryKey') is not null
	alter table log4.JournalControl drop constraint PK_JournalControl;
go
if indexproperty(object_id(N'log4.JournalControl'), N'CI_JournalControl_ModuleName', N'IndexID') is not null
	drop index log4.JournalControl.CI_JournalControl_ModuleName;
go

if objectpropertyex(object_id(N'log4.PK_JournalDetail'), N'IsPrimaryKey') is not null
	alter table log4.JournalDetail drop constraint PK_JournalDetail;
go

if objectpropertyex(object_id(N'log4.PK_Severity'), N'IsPrimaryKey') is not null
	alter table log4.Severity drop constraint PK_Severity;
go
...
if objectpropertyex(object_id(N'[log4].[JournalControl]'), N'IsUserTable') is not null
	and objectpropertyex(object_id(N'[log4Private].[JournalControl]'), N'IsUserTable') is null
	begin
		alter schema [log4Private] transfer [log4].[JournalControl];
	end
go
...
if objectpropertyex(object_id(N'[log4].[Severity]'), N'IsUserTable') is not null
	and objectpropertyex(object_id(N'[log4Private].[Severity]'), N'IsUserTable') is null
	begin
		alter schema [log4Private] transfer [log4].[Severity];
	end
go
...
if objectpropertyex(object_id(N'[log4].[Exception]'), N'IsUserTable') is not null
	and objectpropertyex(object_id(N'[log4Private].[SqlException]'), N'IsUserTable') is null
	begin
		--! Create the new table structrure
		...
		, constraint PK_SqlException primary key clustered (ExceptionId)
		) ;

		--! Migrate the data from the old table
		set identity_insert log4Private.SqlException on;
		...
		select
			...
			, src.NTUsername
			, src.LoginName
			, src.OriginalLoginName
			, src.SessionLoginTime
		from
			log4.Exception as src
		left join log4Private.SqlException as tgt
			on tgt.ExceptionId = src.ExceptionId
		where
			tgt.ExceptionId is null
...
if objectpropertyex(object_id(N'[log4].[Exception]'), N'IsUserTable') is not null
	and objectpropertyex(object_id(N'[log4Private].[Exception]'), N'IsUserTable') is null
	begin
		alter schema [log4Private] transfer [log4].[Exception];
	end
go

if objectpropertyex(object_id(N'[log4Private].[Exception]'), N'IsUserTable') is not null
	and not exists (select 1 from INFORMATION_SCHEMA.TABLES where TABLE_SCHEMA = 'log4Private' and TABLE_NAME like 'Exception_Migrated%')
	begin
		declare @_NewName nvarchar(50); set @_NewName = 'Exception_Migrated_' + convert(char(8), getdate(), 112);
		exec sp_rename N'[log4Private].[Exception]', @_NewName, N'OBJECT'
	end
go

if objectpropertyex(object_id(N'log4Private.PK_Exception'), N'IsPrimaryKey') is not null
	exec sp_rename N'[log4Private].[PK_Exception]', N'PK_Exception_Migrated', N'OBJECT' ;
go
if objectpropertyex(object_id(N'log4Private.DF_Exception_UtcDate'), N'IsDefaultCnst') is not null
	exec sp_rename N'[log4Private].[DF_Exception_UtcDate]', N'DF_Exception_UtcDate_Migrated', N'OBJECT' ;
go
if objectpropertyex(object_id(N'log4Private.DF_Exception_SystemDate'), N'IsDefaultCnst') is not null
	exec sp_rename N'[log4Private].[DF_Exception_SystemDate]', N'DF_Exception_SystemDate_Migrated', N'OBJECT' ;
go

if objectpropertyex(object_id(N'[log4].[Journal]'), N'IsUserTable') is not null
	and objectpropertyex(object_id(N'[log4Private].[Journal]'), N'IsUserTable') is null
	begin
		alter schema [log4Private] transfer [log4].[Journal];
	end
go

if objectpropertyex(object_id(N'[log4].[JournalDetail]'), N'IsUserTable') is not null
	and objectpropertyex(object_id(N'[log4Private].[JournalDetail]'), N'IsUserTable') is null
	begin
		alter schema [log4Private] transfer [log4].[JournalDetail];
	end
go

--!
--! Reapply any dependencies
--!
...
if object_id('[log4].[SessionInfoOutput]') is not null
	and object_id('[log4Private].[SessionInfoOutput]') is null
	begin
		alter schema [log4Private] transfer [log4].[SessionInfoOutput];
	end
go

if object_id('[log4].[ExceptionReader]') is not null
	and object_id('[log4Utils].[ExceptionReader]') is null
	begin
		alter schema [log4Utils] transfer [log4].[ExceptionReader];
	end
go

if object_id('[log4].[JournalReader]') is not null
	and object_id('[log4Utils].[JournalReader]') is null
	begin
		alter schema [log4Utils] transfer [log4].[JournalReader];
	end
go

if object_id('[log4].[PrintString]') is not null
	and object_id('[log4Utils].[PrintString]') is null
	begin
		alter schema [log4Utils] transfer [log4].[PrintString];
	end
go

if object_id('[log4].[JournalPrinter]') is not null
	and object_id('[log4Utils].[JournalPrinter]') is null
	begin
		alter schema [log4Utils] transfer [log4].[JournalPrinter];
	end
go

if object_id('[log4].[JournalCleanup]') is not null
	and object_id('[log4Utils].[JournalCleanup]') is null
	begin
		alter schema [log4Utils] transfer [log4].[JournalCleanup];
	end
go
...
go
exec sp_addextendedproperty N'MS_Description', N'Returns a user-friendly string describing the time elapsed between start and end time', 'SCHEMA', N'log4', 'FUNCTION', N'FormatElapsedTime', null, null
go
...
--!
--! Add pointers for all the objects that have been moved to maximise backwards compatibility
--!
if (select [compatibility_level] from sys.databases where [name] = db_name()) >= 100
	begin
		if not exists(select * from sys.synonyms where [schema_id] = schema_id('log4') and name = 'JournalControl')
			create synonym log4.JournalControl for log4Private.JournalControl;
		if not exists(select * from sys.synonyms where [schema_id] = schema_id('log4') and name = 'Severity')
			create synonym log4.Severity for log4Private.Severity;
		if not exists(select * from sys.synonyms where [schema_id] = schema_id('log4') and name = 'Journal')
			create synonym log4.Journal for log4Private.Journal;
		if not exists(select * from sys.synonyms where [schema_id] = schema_id('log4') and name = 'JournalDetail')
			create synonym log4.JournalDetail for log4Private.JournalDetail;
		if not exists(select * from sys.synonyms where [schema_id] = schema_id('log4') and name = 'Exception')
			create synonym log4.Exception for log4Private.Exception;
		if not exists(select * from sys.synonyms where [schema_id] = schema_id('log4') and name = 'SessionInfoOutput')
			create synonym log4.SessionInfoOutput for log4Private.SessionInfoOutput;
		if not exists(select * from sys.synonyms where [schema_id] = schema_id('log4') and name = 'JournalReader')
			create synonym log4.JournalReader for log4Utils.JournalReader;
		if not exists(select * from sys.synonyms where [schema_id] = schema_id('log4') and name = 'PrintString')
			create synonym log4.PrintString for log4Utils.PrintString;
		if not exists(select * from sys.synonyms where [schema_id] = schema_id('log4') and name = 'JournalPrinter')
			create synonym log4.JournalPrinter for log4Utils.JournalPrinter;
		if not exists(select * from sys.synonyms where [schema_id] = schema_id('log4') and name = 'ExceptionReader')
			create synonym log4.ExceptionReader for log4Utils.ExceptionReader;
		if not exists(select * from sys.synonyms where [schema_id] = schema_id('log4') and name = 'JournalCleanup')
